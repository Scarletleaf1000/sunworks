...
public abstract class PonderScene {
    ...
    public static final int TEXT_DURATION = 100;
    ...
    /** Right-click prompt holding an item above a block; chain e.g. {@code .whileSneaking()} on the result. */
    public static InputElementBuilder showShiftClickWithItemAt(SceneBuilder scene, SceneBuildingUtil util, BlockPos pos,
                                                          ItemStack item, Pointing pointing, int duration) {
        return scene.overlay().showControls(util.vector().topOf(pos), pointing, duration)
                .rightClick()
                .whileSneaking()
    ...
    /** Right-click prompt holding an item above a block, pointing down for the default duration. */
    public static InputElementBuilder showShiftClickWithItemAt(SceneBuilder scene, SceneBuildingUtil util, BlockPos pos, ItemStack item) {
        return showShiftClickWithItemAt(scene, util, pos, item, Pointing.DOWN, DEFAULT_DELAY);
    }
}
